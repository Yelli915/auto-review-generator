...
사진 최대 3장과 리뷰 대상, 별점, 선택한 키워드를 바탕으로 한국어 리뷰 초안을 생성하는 React + Gemini 웹 애플리케이션입니다.
...
- 이미지 최대 3장 업로드, 리뷰 대상 선택, 별점 선택, 키워드 생성, 리뷰 생성, 클립보드 복사
- 이미지별 자르기 비율 선택과 90도 회전 편집
- 리뷰 글자수 옵션 `short`, `medium`, `long` 서버 반영
...
- 리뷰 글자수 옵션: 짧게(30자 이상), 보통(60자 이상), 길게(90자 이상)
- 말투 옵션: 기본, 친근, 격식, 반말
- 실시간 스트리밍 리뷰 출력
- 생성된 리뷰 클립보드 복사
...
- 업로드 원본 파일은 클라이언트에서 이미지별 15MB 이하, 한 리뷰당 최대 3장으로 제한합니다.
...
프론트엔드는 `short`, `medium`, `long` 값을 서버에 그대로 보냅니다. 리뷰 글자수별 프롬프트 문구와 최소 글자 수 검증은 서버에서 관리합니다.
...
- 한 리뷰당 최대 3장 업로드
- 클라이언트 리사이징 적용
- 이미지별 원본/정사각형/4:3 자르기와 90도 회전 적용
- API 요청 전 이미지 압축 처리
...
- 선택한 대상, 키워드, 별점, 리뷰 글자수, 말투 옵션을 기반으로 리뷰를 생성합니다.
